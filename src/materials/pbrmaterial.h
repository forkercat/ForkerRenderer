...
public:
    ...
    explicit PBRMaterial(std::string name)
        : name(std::move(name)), ka(0.f), albedo(1.f), roughness(0.f), metalness(0.f),
...
inline std::ostream& operator<<(std::ostream& out, const PBRMaterial& m)
{
    out << StringPrintf(
        ...
        "Ka(%3.2f, %3.2f, %3.2f) Albedo(%3.2f, %3.2f, %3.2f) Roughness(%3.2f) Metalness(%3.2f)",
        m.baseColorMap ? "o" : "x", m.roughnessMap ? "o" : "x",
        m.metalnessMap ? "o" : "x", m.ambientOcclusionMap ? "o" : "x",
        m.normalMap ? "o" : "x", m.emissiveMap ? "o" : "x",
        (Float) m.ka.x, (Float) m.ka.y, (Float) m.ka.z, (Float)m.albedo.x, (Float)m.albedo.y, (Float)m.albedo.z,
